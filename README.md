...
- **Screenshot import** — snap the in-game player-profile screen and the app reads name, position, age, quality and all 15 attributes with on-device OCR (bundled Tesseract, ~15 MB lazy-loaded on first use, works offline after; nothing is uploaded). After reading, choose **update an existing player** (fuzzy name match preselected; unread skills keep their saved values) or **add as new** — recognised values prefill the editor for review either way.
...
- **Development tracker** — every import/edit snapshots a player's quality and attributes, so each player page shows a **growth curve**, **quality gained per week** (recent and overall), and their **biggest-moving skills**; the Squad page rolls this up into **rising / slipping** lists. Re-import your squad from a recording periodically and the numbers track themselves — the trailing-window rate stays meaningful even across a season −20 reset.
