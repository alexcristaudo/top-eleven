...
- **Playstyles & special abilities** — per-player playstyle tracking with the drills that level each style, recommended playstyles per position, and a squad-wide **special-ability coverage checker** against the community-recommended kit (SAs are read automatically by the OCR importers).
